...
def save_plot(out_path: pathlib.Path, frequencies: np.ndarray, magnitude_db: np.ndarray) -> None:
    ...
    plt.xlim(10.0, 500.0)
    plt.ylim(np.min(magnitude_db[(frequencies >= 10.0) & (frequencies <= 500.0)]) - 3.0,
...
def main() -> None:
    ...
    dry, dry_sample_rate = read_wav_mono(pathlib.Path(args.dry))
    wet, wet_sample_rate = read_wav_mono(pathlib.Path(args.wet))

    if dry_sample_rate != wet_sample_rate:
        raise ValueError(f"Sample rate mismatch: dry={dry_sample_rate}, wet={wet_sample_rate}")

    aligned_wet = align_impulse(dry, wet)
    frequencies, magnitude_db = compute_frequency_response(aligned_wet, wet_sample_rate)
    ...
    if out_path.suffix.lower() == ".json":
        out_path.write_text(json.dumps({"sampleRate": wet_sample_rate, "pointsDb": points}, indent=2))
    else:
        save_plot(out_path, frequencies, magnitude_db)
